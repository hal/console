...
import org.jboss.hal.client.bootstrap.hal.ErraiBootstrapper.BootstrapExceptionCallback;
import org.jboss.hal.client.bootstrap.hal.ErraiBootstrapper.BootstrapFailedCallback;
...
public class ReadEnvironment implements Function<BootstrapContext> {
    ...
    public void execute(final Control<BootstrapContext> control) {
        ...
        dispatcher.execute(composite, payload -> {

            logger.debug("Bootstrap[ReadEnvironment]: Parse root resource");

            // server info
            ModelNode node = payload.get("step-1").get(RESULT);
            environment.setInstanceInfo(node.get("product-name").asString(), node.get("product-version").asString(),
...
//            control.proceed();
        }, new BootstrapFailedCallback(control), new BootstrapExceptionCallback(control));
    }
}
