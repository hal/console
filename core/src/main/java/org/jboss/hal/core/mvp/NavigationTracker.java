...
public class NavigationTracker implements NavigationHandler, FinderContextHandler, ModelBrowserPathHandler {
    ...
    private static final Set<String> IGNORE_PLACES = Sets.newHashSet(
            ...
            NameTokens.PATCHING,
            NameTokens.RUNTIME);
    @NonNls private static final Logger logger = LoggerFactory.getLogger(NavigationTracker.class);

    private final GoogleAnalytics ga;

    public NavigationTracker(GoogleAnalytics ga) {
        this.ga = ga;
    }
    ...
    public void onNavigation(NavigationEvent event) {
        ...
        if (!IGNORE_PLACES.contains(request.getNameToken())) {
